export const expertise = [
  {
    icon: 'code',
    title: 'Website Development',
    desc: 'Fast, responsive websites that are easy to use and ready for search.',
    accent: 'primary',
    kicker: '01',
    metric: 'Web',
    points: ['Custom websites', 'eCommerce and WordPress', 'SEO-friendly structure'],
  },
  {
    icon: 'phone_iphone',
    title: 'App Development',
    desc: 'Android and iOS apps planned, designed, built, and supported after launch.',
    accent: 'secondary',
    kicker: '02',
    metric: 'Mobile',
    points: ['Native apps', 'Cross-platform builds', 'Maintenance and updates'],
  },
  {
    icon: 'campaign',
    title: 'Digital Marketing',
    desc: 'SEO, content, social media, and ad campaigns focused on better leads.',
    accent: 'tertiary',
    kicker: '03',
    metric: 'Growth',
    points: ['SEO optimization', 'Social media marketing', 'PPC campaigns'],
  },
  {
    icon: 'chat',
    title: 'WhatsApp Business API',
    desc: 'WhatsApp API setup, chat flows, broadcasts, and customer support handoff.',
    accent: 'primary',
    kicker: '04',
    metric: 'Messaging',
    points: ['Official API setup', 'Chatbot flows', 'Broadcast analytics'],
  },
  {
    icon: 'design_services',
    title: 'UI / UX Design',
    desc: 'Clear screens, simple user journeys, and prototypes your team can review.',
    accent: 'secondary',
    kicker: '05',
    metric: 'Experience',
    detail: 'Good for wireframes, clickable prototypes, and design handoff for developers.',
    points: ['User research', 'High-fidelity prototypes', 'Design-system handoff'],
  },
  {
    icon: 'palette',
    title: 'Graphics Design',
    desc: 'Brand graphics for logos, campaigns, social media, print, and ads.',
    accent: 'tertiary',
    kicker: '06',
    metric: 'Brand',
    points: ['Brand identity', 'Campaign creatives', 'Social and print assets'],
  },
];

export const stats = [
  { value: 150, suffix: '+', label: 'Projects completed' },
  { value: 100, suffix: '+', label: 'Clients' },
  { value: 96, suffix: '%', label: 'Client satisfaction' },
  { value: 5, suffix: '+ yrs', label: 'Experience' },
];

export const methodology = [
  {
    step: '01',
    title: 'Discover',
    desc: 'We understand your goals, users, and current problems before we build.',
  },
  {
    step: '02',
    title: 'Design',
    desc: 'We create wireframes, screens, and reusable design rules.',
  },
  {
    step: '03',
    title: 'Build',
    desc: 'We build in clear steps and show real progress often.',
  },
  {
    step: '04',
    title: 'Iterate',
    desc: 'We use feedback and data to improve the product after launch.',
  },
];

export const portfolio = [
  {
    title: 'Website development systems',
    tag: 'Web + SEO',
    image:
      'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'AI chatbot workspaces',
    tag: 'AI + Support',
    image:
      'https://images.unsplash.com/photo-1620712943543-bcc4688e7485?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'UI/UX design systems',
    tag: 'Design + Prototype',
    image:
      'https://images.unsplash.com/photo-1559028012-481c04fa702d?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'Mobile app builds',
    tag: 'iOS + Android',
    image:
      'https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'Digital marketing funnels',
    tag: 'SEO + PPC',
    image:
      'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=1200&q=80&auto=format&fit=crop',
  },
];

export const techStack = [
  'React',
  'Angular',
  'Vue.js',
  'Node.js',
  'Python',
  'PHP',
  'Java',
  'MySQL',
  'MongoDB',
  'AWS',
  'Docker',
  'Kubernetes',
  'Git',
  'Jenkins',
  'Azure',
  'Firebase',
  'GraphQL',
  'TypeScript',
];

export const usefulHighlights = [
  {
    icon: 'public',
    title: 'Web platforms',
    desc: 'Responsive websites, eCommerce stores, WordPress sites, and SEO-ready web apps.',
  },
  {
    icon: 'phone_iphone',
    title: 'Mobile apps',
    desc: 'Native and cross-platform apps with clear design and long-term support.',
  },
  {
    icon: 'smart_toy',
    title: 'AI enablement',
    desc: 'Chatbots, AI integrations, automation, analytics dashboards, and reports.',
  },
  {
    icon: 'campaign',
    title: 'Growth systems',
    desc: 'SEO, social media, paid ads, content, and WhatsApp Business API support.',
  },
];
