export const expertise = [
  {
    icon: 'code',
    title: 'Website Development',
    desc: 'Custom websites built for performance and user experience.',
    accent: 'primary',
    path: '/services/website-development',
    points: ['Custom Web Development', 'eCommerce Solutions', 'WordPress Development', 'Responsive Design'],
  },
  {
    icon: 'phone_iphone',
    title: 'App Development',
    desc: 'Mobile applications for iOS and Android platforms.',
    accent: 'secondary',
    path: '/services/app-development',
    points: ['Native App Development', 'Cross-platform Solutions', 'App UI/UX Design', 'App Maintenance'],
  },
  {
    icon: 'campaign',
    title: 'Digital Marketing',
    desc: 'Strategic campaigns to boost your online presence.',
    accent: 'tertiary',
    path: '/services/digital-marketing',
    points: ['SEO Optimization', 'Social Media Marketing', 'PPC Campaigns', 'Content Marketing'],
  },
  {
    icon: 'chat',
    title: 'WhatsApp Business API',
    desc: 'Enterprise-grade WhatsApp solutions for business communication.',
    accent: 'primary',
    path: '/services/whatsapp-business',
    points: ['Business Account Setup', 'Automated Responses', 'Broadcast Messages', 'Analytics Dashboard'],
  },
  {
    icon: 'design_services',
    title: 'UI / UX Design',
    desc: 'Creative and intuitive user interface design.',
    accent: 'secondary',
    path: '/services/ui-ux-design',
    points: ['User Research', 'Interface Design', 'Prototyping', 'Usability Testing'],
  },
  {
    icon: 'palette',
    title: 'Graphics Design',
    desc: 'Eye-catching visual designs for your brand.',
    accent: 'tertiary',
    path: '/services/graphics-designing',
    points: ['Brand Identity', 'Marketing Materials', 'Social Media Graphics', 'Print Design'],
  },
];

export const stats = [
  { value: 150, suffix: '+', label: 'Projects completed' },
  { value: 100, suffix: '+', label: 'Clients' },
  { value: 96, suffix: '%', label: 'Client satisfaction' },
  { value: 5, suffix: '+ yrs', label: 'Experience' },
];

export const methodology = [
  {
    step: '01',
    title: 'Discover',
    desc: 'We understand your goals, users, and current problems before we build.',
  },
  {
    step: '02',
    title: 'Design',
    desc: 'We create wireframes, screens, and reusable design rules.',
  },
  {
    step: '03',
    title: 'Build',
    desc: 'We build in clear steps and show real progress often.',
  },
  {
    step: '04',
    title: 'Iterate',
    desc: 'We use feedback and data to improve the product after launch.',
  },
];

export const portfolio = [
  {
    title: 'Business Website + SEO Build',
    tag: 'Web + SEO',
    desc: 'A fast business website structure with clear service pages, local SEO basics, lead forms, and performance-friendly layouts for better discovery.',
    details: ['Responsive pages', 'Lead capture', 'Search-ready content'],
    image:
      'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'AI Support Chatbot Workspace',
    tag: 'AI + Support',
    desc: 'A customer support assistant trained on business content, built to answer common questions, collect leads, and hand off complex chats.',
    details: ['FAQ replies', 'Lead routing', 'Human handoff'],
    image:
      'https://images.unsplash.com/photo-1620712943543-bcc4688e7485?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'Product UI/UX Design',
    tag: 'Design + Prototype',
    desc: 'Screen flows, wireframes, prototypes, and reusable UI rules that help teams understand the product before development starts.',
    details: ['User flows', 'Prototype screens', 'Design handoff'],
    image:
      'https://images.unsplash.com/photo-1559028012-481c04fa702d?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'Mobile App Development',
    tag: 'iOS + Android',
    desc: 'Mobile app planning and development for Android and iOS, covering core user journeys, launch support, updates, and maintenance.',
    details: ['App screens', 'API integration', 'Launch support'],
    image:
      'https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=1200&q=80&auto=format&fit=crop',
  },
  {
    title: 'Digital Growth Funnel',
    tag: 'SEO + PPC',
    desc: 'Landing pages, campaign tracking, SEO improvements, and paid ad funnels shaped around better leads and clearer reporting.',
    details: ['Campaign pages', 'Analytics setup', 'Lead tracking'],
    image:
      'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=1200&q=80&auto=format&fit=crop',
  },
];

export const techStack = [
  'React',
  'Angular',
  'Vue.js',
  'Node.js',
  'Python',
  'PHP',
  'Java',
  'MySQL',
  'MongoDB',
  'AWS',
  'Docker',
  'Kubernetes',
  'Git',
  'Jenkins',
  'Azure',
  'Firebase',
  'GraphQL',
  'TypeScript',
];

export const usefulHighlights = [
  {
    icon: 'public',
    title: 'Web platforms',
    desc: 'Responsive websites, eCommerce stores, WordPress sites, and SEO-ready web apps.',
  },
  {
    icon: 'phone_iphone',
    title: 'Mobile apps',
    desc: 'Native and cross-platform apps with clear design and long-term support.',
  },
  {
    icon: 'smart_toy',
    title: 'AI enablement',
    desc: 'Chatbots, AI integrations, automation, analytics dashboards, and reports.',
  },
  {
    icon: 'campaign',
    title: 'Growth systems',
    desc: 'SEO, social media, paid ads, content, and WhatsApp Business API support.',
  },
];
